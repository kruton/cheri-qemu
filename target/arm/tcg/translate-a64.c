                                                  ...
                                                  bool is_zva)
{
    ...
    if (!is_zva) {
        ...
        // after
        tcg_gen_st8_i32(tcg_constant_i32(1), cpu_env, offsetof(CPUARMState, exception.cm));
    }
    ...
    if (!is_zva) {
        tcg_gen_st8_i32(tcg_constant_i32(0), cpu_env, offsetof(CPUARMState, exception.cm));
    }
...
static bool trans_STZGM(DisasContext *s, arg_ldst_tag *a)
{
    ...
    if (s->ata[0]) {
        gen_helper_stzgm_tags(cpu_env, (TCGv_cap_checked_ptr)addr, tcg_rt);
    }
    /*
    ...
    gen_helper_dc_zva(cpu_env, (TCGv_cap_checked_ptr)clean_addr);
    return true;
}
