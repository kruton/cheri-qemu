...
#else
    env->menvcfg = (cpu->cfg.ext_svadu ? MENVCFG_HADE : 0);
    env->henvcfg = (cpu->cfg.ext_svadu ? HENVCFG_HADE : 0);
#endif
...
static void riscv_cpu_validate_set_extensions(RISCVCPU *cpu, Error **errp)
{
    ...
    if (cpu->cfg.ext_icbom) {
        if (!is_power_of_2(cpu->cfg.cbom_blocksize)) {
            error_setg(errp, "cbom_blocksize must be a power of 2");
            return;
        }
    }
    if (cpu->cfg.ext_icboz) {
        if (!is_power_of_2(cpu->cfg.cboz_blocksize)) {
            error_setg(errp, "cboz_blocksize must be a power of 2");
            return;
        }
    }
#ifdef TARGET_CHERI
