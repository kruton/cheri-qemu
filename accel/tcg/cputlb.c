...
static bool victim_tlb_hit(CPUArchState *env, size_t mmu_idx, size_t index,
                           MMUAccessType access_type, target_ulong page, bool cap_write)
{
    size_t vidx;
                                ...
                                uintptr_t retaddr)
{
    ...
    if (MO_TE == MO_LE) {
        return helper_le_lduw_mmu(env, ptr, oi, retaddr);
    } else {
        return helper_be_lduw_mmu(env, ptr, oi, retaddr);
    }
#elif TARGET_LONG_BITS == 64
    oi = make_memop_idx(MO_TEUQ, cpu_mmu_index(env, false));
    if (MO_TE == MO_LE) {
        return helper_le_ldq_mmu(env, ptr, oi, retaddr);
    } else {
        return helper_be_ldq_mmu(env, ptr, oi, retaddr);
    }
#else
                        ...
                        target_ulong val, uintptr_t retaddr)
{
    ...
    oi = make_memop_idx(MO_TEUW, cpu_mmu_index(env, false));
    if (MO_TE == MO_LE) {
        helper_le_stw_mmu(env, ptr, val, oi, retaddr);
    } else {
        helper_be_stw_mmu(env, ptr, val, oi, retaddr);
    }
#elif TARGET_LONG_BITS == 64
    oi = make_memop_idx(MO_TEUQ, cpu_mmu_index(env, false));
    if (MO_TE == MO_LE) {
        helper_le_stq_mmu(env, ptr, val, oi, retaddr);
    } else {
        helper_be_stq_mmu(env, ptr, val, oi, retaddr);
    }
#else
#error "Unhandled target long width"
#endif
